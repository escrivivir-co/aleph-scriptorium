# Backlog — Fundación

> **Opportunity**: Fundación  
> **Versión**: 0.0.1  
> **Sprint actual**: 0 (Bootstrap)

---

## Épica: FUND-0.0.1 — Consolidación del ARCHIVO

**Objetivo**: Establecer una base doctrinal sólida antes de comenzar la redacción serializada.

**Entregables**:
- ARCHIVO/marco completo (docs 01–11)
- ARCHIVO/diagnostico completo (docs 01–05)
- Índice de 12 capítulos validado
- Indicadores de fracaso para cap. 1

---

## Stories

### FUND-0.0.1-S01: Marco Conceptual
**Estado**: ✅ Completada

| Task ID | Descripción | Estado |
|---------|-------------|--------|
| T001 | Crear `08-trabajo-campo-batalla.md` | ✅ |
| T002 | Crear `09-teologia-politica-imperial.md` | ✅ |
| T003 | Crear `10-euroamerica-deuda.md` | ✅ |
| T004 | Crear `11-tecnofeudalismo-critica-puentes.md` | ✅ |
| T005 | Actualizar `ARCHIVO/marco/README.md` | ✅ |

---

### FUND-0.0.1-S02: Diagnóstico
**Estado**: ✅ Completada

| Task ID | Descripción | Estado |
|---------|-------------|--------|
| T006 | Crear `05-carisma-vs-responsabilidad.md` | ✅ |
| T007 | Actualizar `ARCHIVO/diagnostico/README.md` | ✅ |

---

### FUND-0.0.1-S03: Planificación Anual
**Estado**: 🔄 En progreso

| Task ID | Descripción | Estado |
|---------|-------------|--------|
| T008 | Revisar `PROYECTOS/FUNDACION/Indice.md` | ✅ |
| T009 | Crear `indicadores-fracaso-enero.md` | ✅ |
| T010 | Validar estructura de 12 capítulos | 🔄 |

---

### FUND-0.0.1-S04: Capítulo 1 (Prep)
**Estado**: 🔄 En progreso (Sprint 1)

| Task ID | Descripción | Estado |
|---------|-------------|--------|
| T011 | Borrador inicial `cap01-anacronismo-productivo.md` | ⏳ |
| T011.1 | Integración Aleph Cap01 (`01.enero.aleph.handson.md`) | ✅ |
| T012 | Revisión interna | ⏳ |
| T013 | Definir sacrificio y sombra | ⏳ |

---

### FUND-0.1.0-S01: Auditoría de Sombras (Bandera Negra)
**Estado**: 🔄 En progreso (Sprint 1)

| Task ID | Descripción | Estado |
|---------|-------------|--------|
| T015 | Auditoría de Sombras y afilado de dilemas (Arco Enero) | 🔄 |
| T015.1 | Auditoría Blackflag Cap01 (`01.enero.black.handson.md`) | ✅ |
| T016 | Validación de `indicadores-fracaso-enero.md` (Posverdad técnica) | ✅ |

---

### FUND-0.1.0-S02: Auditoría de Estructura (Bandera Roja)
**Estado**: 🔄 En progreso (Sprint 1)

| Task ID | Descripción | Estado |
|---------|-------------|--------|
| T017 | Auditoría de Estructura y Viabilidad (Arco Enero) | ⏳ |
| T017.1 | Auditoría Redflag Cap01 (`01.enero.red.handson.md`) | ✅ |
| T018 | Inyección de Tests de Escala y Coerción en `indicadores-fracaso-enero.md` | ✅ |
| T019 | Crear agente `redflag.agent.md` | ✅ |
| T020 | Crear agente `blueflag.agent.md` | ✅ |
| T021 | Inyección de Tests de Verdad (Blueflag) en `indicadores-fracaso-enero.md` | ✅ |

---

### FUND-0.1.0-S03: Auditoría de Verdad (Bandera Azul)
**Estado**: 🔄 En progreso (Sprint 1)

| Task ID | Descripción | Estado |
|---------|-------------|--------|
| T024 | Auditoría Blueflag Cap01 (`01.enero.blue.handson.md`) | ✅ |

---

### FUND-0.0.1-S08: Material de Coyuntura (DISCO)
**Estado**: ✅ Completada

| Task ID | Descripción | Estado |
|---------|-------------|--------|
| T025 | Archivar análisis de coyuntura (Assange, Trump, Conexiones) en `ARCHIVO/DISCO/` | ✅ |

---

### FUND-0.0.1-S05: Banderas Roja y Negra (Hibridación)
**Estado**: ✅ Completada

| Task ID | Descripción | Estado |
|---------|-------------|--------|
| T014 | Hibridar arquetipos de praxis en los tres ejes | ✅ |

---

### FUND-0.0.1-S06: Cartas-puerta (tutoría)
**Estado**: ✅ Completada

| Task ID | Descripción | Estado |
|---------|-------------|--------|
| T022 | Crear cartas-puerta en `ARCHIVO/CARTAS/` | ✅ |

---

### FUND-0.0.1-S07: Perfiles de lector
**Estado**: ✅ Completada

| Task ID | Descripción | Estado |
|---------|-------------|--------|
| T023 | Crear `ARCHIVO/PERFILES/` con README | ✅ |

---

## Métricas Sprint 0

| Métrica | Valor |
|---------|-------|
| Tasks totales | 27 |
| Completadas | 23 |
| En progreso | 2 |
| Pendientes | 2 |
| % Avance | 85% |

---

## Dependencias con Scriptorium

| Dependencia | Estado |
|-------------|--------|
| Protocolo DevOps | ✅ |
| Agente Aleph actualizado | ✅ |
| Prompt de extracción | ✅ |
| Agentes auditores (Blackflag, Redflag) | ✅ |

---

## Changelog

| Fecha | Cambio | Autor |
|-------|--------|-------|
| 2024-12-19 | Creación inicial del backlog | Aleph |
| 2024-12-19 | Completar docs marco 08–11 | Aleph |
| 2024-12-19 | Completar diagnostico 05 | Aleph |
| 2024-12-19 | Refinar tecnofeudalismo (dos capas) | Aleph |
| 2024-12-19 | Hibridar "Banderas Roja y Negra" en el ARCHIVO | Aleph |
| 2025-12-19 | Crear agente Bandera Roja (`redflag.agent.md`) | Aleph |
| 2025-12-19 | Añadir Auditoría de Sombras (T015-T016) | Blackflag |
| 2025-12-19 | Añadir Auditoría de Estructura (T017-T019) | Redflag |
| 2025-12-19 | Crear agente Bandera Azul (`blueflag.agent.md`) | Aleph |
| 2025-12-19 | Inyectar Tests de Escala, Coerción, Suministro (T018) | Redflag |
| 2025-12-19 | Inyectar Tests de Evidencia, Utilidad, Falsificabilidad (T021) | Blueflag |
| 2025-12-19 | Completar Auditoría Blackflag Cap01 (T015.1) — 5 refactorizaciones propuestas | Blackflag |
| 2025-12-19 | Completar Auditoría Redflag Cap01 (T017.1) — escala/enforcement/base material | Redflag |
| 2025-12-19 | Completar Auditoría Blueflag Cap01 (T024) — evidencia/utilidad/falsificabilidad | Blueflag |
| 2025-12-19 | Integración Aleph Cap01 (T011.1) — plan de refactorización | Aleph |
