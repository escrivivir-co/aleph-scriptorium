...
> Sirve como mapa de contexto, evidencias, onboarding y siguientes pasos a
> partir del broadcast corto que vive en BotHubSDK.
...
- `sala/dossiers/scriptorium-bot-hub-integration/archivo/broadcast/proxy-retro.md` = mapa humano de contexto y enlaces complementarios.
...
- Confirmar si la siguiente sesion se abre con `mock crypto`, `firma real` o `sala de staging`.
- Tomar `grafo-sdk` / `DocumentMachineSDK` como foco de asesoria.
- Si hay OK, abrir el siguiente dossier para el white paper `WHITE PAPER RETRO GRAFO SDK for SCRIPTORIUM BOTHUBSDK`.